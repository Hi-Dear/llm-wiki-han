...
#### T-등가회로 상세 구조 (Sahin 2013)

일반 원통형 요소 하나(예: 고정자 요크)를 **축방향 T-망**과 **반경방향 T-망** 두 개로 독립적으로 모델링한다(원주방향 열류는 무시). 각 T-망은 저항 3개로 구성 — 두 개는 요소의 양쪽 경계온도($T_{axial,left}/T_{axial,right}$ 또는 $T_{radial,in}/T_{radial,out}$)로 이어지는 "다리"이고, 세 번째($R_3$, 부호가 음수일 수 있음)는 요소의 평균온도 $T_m$ 노드로 이어져 내부 발열 $P$와 열용량 $C$(과도해석 시)가 주입되는 지점이다. 정상상태에서는 $C$를 생략.

고정자 요크를 예로 든 실제 공식(원통 외경 $r_{out}$, 내경 $r_{in}$, 두께 $l_{yoke}$, 적층강판의 축방향/반경방향 열전도율 $k_{core\_a}$, $k_{core\_r}$):

**축방향(axial)**
$$R_{1a}=R_{2a}=\frac{l_{yoke}}{2\pi k_{core\_a}(r_{out}^2-r_{in}^2)},\qquad R_{3a}=\frac{-l_{yoke}}{6\pi k_{core\_a}(r_{out}^2-r_{in}^2)}$$

**반경방향(radial)**
$$R_{1r}=\frac{1}{4\pi k_{core\_r} l_{yoke}}\left[1-\frac{2r_{in}^2\ln(r_{out}/r_{in})}{r_{out}^2-r_{in}^2}\right]$$
$$R_{2r}=\frac{1}{4\pi k_{core\_r} l_{yoke}}\left[\frac{2r_{out}^2\ln(r_{out}/r_{in})}{r_{out}^2-r_{in}^2}-1\right]$$
$$R_{3r}=\frac{-1}{8\pi k_{core\_r} l_{yoke}(r_{out}^2-r_{in}^2)}\left[r_{out}^2+r_{in}^2-\frac{4r_{out}^2 r_{in}^2\ln(r_{out}/r_{in})}{r_{out}^2-r_{in}^2}\right]$$

같은 방식(치수·재질 대입)을 권선·자석·회전자 디스크 등 다른 원통형 요소에도 반복 적용한다. 적층강판은 면내(축방향) 열전도가 면외(반경방향)보다 훨씬 큼(코팅층+적층률 0.92 탓) — 위 두 방향 저항식을 따로 두는 이유가 여기서 직접 확인됨.

**대류 저항**: 공극·프레임-주변 등 공기층 경로는 단일 저항 $R_c = 1/(hS)$로 모델링($h$=대류계수, $S$=열전달 표면적). 공극 대류계수 $h_{agap}$는 위 Taylor수 상관식으로 구한 Nu수를 이용해 $h_{agap}=Nu\,\lambda_{air}/g$ ($g$=공극 길이, $\lambda_{air}$=공기 열전도율).

**방사 저항**: 기기 내부 표면 간 방사는 온도차가 작아 무시되지만, 프레임 표면-주변 사이는 등가 방사계수 $\alpha_{rad}=P_{rad}/(T_{sur}-T_{amb})$로 대류저항과 동일한 형태로 반영. $P_{rad}=\varepsilon_{sur}\sigma_{SB}(T_{sur}^4-T_{amb}^4)$ — 표면 방사율 $\varepsilon_{sur}$는 흑색 도장 시 약 0.9, 연마 강판 시 0.3 미만으로 재질 의존성이 큼. 소형기일수록 방사 비중이 무시 못 할 수준.

**전체 노드 구성**: Sahin(2013)의 DSSR(이중고정자-단일회전자) 기기는 축대칭을 이용해 절반만 모델링하고, 13개 열노드로 분할 — ① 고정자 슬롯권선 ② 외경측 단부권선 ③ 내경측 단부권선 ④ 고정자 치 ⑤ 고정자 요크 ⑥ 영구자석 ⑦ 회전자 디스크 ⑧ 공극 ⑨ 엔드캡 공기 ⑩ 고정판(fixing plate) ⑪ 베어링실드 ⑫ 프레임 ⑬ 샤프트. 고정자 요크-고정판 접합부, 베어링실드-고정판 접촉부, 자석-회전자디스크 접착층에는 별도의 접촉저항을 추가로 삽입.

**손실 주입 위치**: 동손은 슬롯 내 권선/외경측 단부권선/내경측 단부권선 3곳에 분리 주입(온도 의존적이라 반복계산으로 보정), 철손은 요크·치에 분리 주입(반경별 치 두께가 달라 Quasi-3D로 계산), 베어링손은 별도 노드, PM·회전자디스크 와전류손은 저속기에서는 통상 무시하나 고속기에서는 반드시 포함.
